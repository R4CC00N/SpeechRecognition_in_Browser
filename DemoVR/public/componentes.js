...
AFRAME.registerComponent('input-text', {
  ...
  init: function () {
    ...
    // WebSocket para enviar datos al servidor
    const SERVER_IP = "192.168.0.231";  // Cambia con la IP de tu servidor
    socket = new WebSocket(`wss://${SERVER_IP}:8080`);
        ...
        .then((stream) => {
          ...
          mediaRecorder.ondataavailable = (event) => {
            if (event.data.size > 0) {
              socket.send(event.data);  // Enviar cada fragmento de audio
            }
          };

          mediaRecorder.onstop = () => {
            socket.send("END");  // Enviar "END" cuando se detenga la grabación
          };

          mediaRecorder.start(1000);  // Enviar audio cada 1 segundo
    ...
    // Funciones para procesar la transcripción
    ...
    // Recibir la transcripción del servidor WebSocket y procesarla
    socket.onmessage = (event) => {
      ...
      if (data.text) {
        ...
        transcript = quitarAcentos(transcript); // Eliminar acentos
        transcript = convertirTextoANumero(transcript); // Convertir texto a número

        // Aquí podrías enviar la transcripción procesada a otro componente o hacer lo que necesites
